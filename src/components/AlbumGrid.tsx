...
const AlbumCard = memo(
  ({
    ...
    return (
      ...
      >
        {hasError ? (
          <div className="aspect-square w-full bg-gray-100 dark:bg-gray-800/50 flex flex-col items-center justify-center p-4 text-center border-b border-gray-200 dark:border-gray-700">
            <svg
        ...
        <div
          className={`absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-black/95 via-black/40 to-transparent p-4 transition-opacity duration-300 ${isSelected ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}
        >
          ...
          <div className="flex justify-between items-center mt-2">
            <span className="text-[9px] text-blue-400 font-black uppercase tracking-widest bg-blue-400/10 px-1.5 py-0.5 rounded">
              {album.provider}
            </span>
            {!isSelected && (
    ...
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        {[...Array(10)].map((_, i) => (
          <div
            key={i}
            className="aspect-square w-full rounded-xl bg-gray-100 dark:bg-gray-900 animate-pulse border border-gray-200 dark:border-gray-800"
          />
        ))}
      </div>
    );
