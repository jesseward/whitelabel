...
export class LastFmProvider extends BaseProvider {
  ...
  async search(params: SearchParams, page = 1): Promise<SearchResult> {
    const apiKey = useSettingsStore.getState().apiKeys.lastfm || import.meta.env.VITE_LASTFM_API_KEY;
    ...
    const data = await this.fetchJson<LastFmSearchResponse>(`${this.baseUrl}?${queryParams.toString()}`);
    
    const albums = data.results.albummatches.album.map((album): AlbumArt => ({
      id: `${this.name}-${album.mbid || album.url}`,
      url: album.image.find((img) => img.size === 'extralarge')?.['#text'] || album.image[0]?.['#text'] || '',
      artist: album.artist,
      album: album.name,
      provider: this.name
    }));
