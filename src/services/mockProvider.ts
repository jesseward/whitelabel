...
export const MockProviderService = {
  search: async (params: SearchParams): Promise<SearchResult> => {
    ...
    const filtered = MOCK_ALBUMS.filter(album => {
      if (artistQuery && !album.artist.toLowerCase().includes(artistQuery)) return false;
      if (albumQuery && !album.album.toLowerCase().includes(albumQuery)) return false;
      if (!artistQuery && !albumQuery) {
        return album.artist.toLowerCase().includes(query) || album.album.toLowerCase().includes(query);
      }
      return true;
    });
